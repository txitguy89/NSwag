...
namespace NSwag.CodeGeneration.OperationNameGenerators
{
    ...
    public class MultipleClientsFromFirstTagAndPathSegmentsOperationNameGenerator : OperationNameGeneratorBase, IOperationNameGenerator
    {
        ...
        public override string GetOperationName(OpenApiDocument document, string path, string httpMethod, string mediaType, OpenApiOperation operation)
        {
            ...
            var hasNameConflict = document.Paths
                .SelectMany(pair => pair.Value.Select(p => new { Path = pair.Key.Trim('/'), HttpMethod = p.Key, p.Value.Produces, Operation = p.Value }))
                .Where(op =>
